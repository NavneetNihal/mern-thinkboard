...
function RoastModal({ isOpen, text, gifUrl, mediaType, soundUrl, soundName, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/85 backdrop-blur-md p-4 animate-fade-in">
      <div
        className="bg-neutral-900 border border-solid border-red-500/30 shadow-[0_0_50px_rgba(255,59,48,0.25)] rounded-2xl max-w-md w-full p-6 flex flex-col items-center text-center gap-4 relative overflow-hidden"
        style={{
          background:
            "radial-gradient(125% 125% at 50% 10%, #151515 50%, rgba(255,59,48,0.08) 100%)",
        }}
      >
        ...
        {/* Header Title */}
        <div className="flex items-center gap-2 text-red-500 font-extrabold text-2xl uppercase tracking-widest animate-pulse">
            ...
            <span className="flex items-center gap-1.5 text-[11px] font-bold tracking-wider px-3 py-1 rounded-full bg-red-500/20 border border-red-500/40 text-red-500">
              ...
              {isPlaying && !isMuted && (
                <span className="flex items-end gap-0.5 h-3 ml-1">
                  <span className="w-0.5 bg-red-500 h-full animate-pulse"></span>
                  <span className="w-0.5 bg-red-500 h-2/3 animate-ping"></span>
        ...
        <div className="min-h-16 flex items-center justify-center px-2 py-1">
          <p className="text-base text-white font-medium leading-relaxed italic">
            &quot;{displayedText}&quot;
            <span className="inline-block w-1.5 h-4 ml-1 bg-red-500 animate-pulse"></span>
          </p>
        </div>
        ...
        <button
          type="button"
          onClick={onClose}
          className="btn btn-error w-full font-bold tracking-widest uppercase transition-all duration-200 shadow-md hover:shadow-lg hover:scale-[1.02] border-none bg-gradient-to-r from-red-500 to-red-600 text-white"
        >
